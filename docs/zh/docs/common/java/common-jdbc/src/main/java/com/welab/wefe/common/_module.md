...
| 名称 | common |
| 编码语言 | .java |
| 代码路径 | WeFe/common/java/common-jdbc/src/main/java/com/welab/wefe/common |
| 包名 | docs.common.java.common-jdbc.src.main.java.com.welab.wefe.common |
...
该流程图展示了WeFe项目中common-jdbc模块的层级结构。从根目录common开始，分支到jdbc目录，jdbc下进一步分为hive、mysql和base三个子目录。每个子目录包含对应的扫描器实现类，如HiveScanner.java和MysqlScanner.java。base目录包含数据库类型和基础扫描器类，jdbc目录下还直接包含JdbcClient.java文件。整体结构清晰展示了不同数据库类型的功能划分。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [jdbc](jdbc/_module.md) | package | HiveScanner和MysqlScanner是JdbcScanner的子类，分别用于Hive和MySQL查询。JdbcScanner是抽象基类，封装JDBC查询流程，支持多数据库适配。JdbcClient封装JDBC操作，支持多种数据库，提供连接管理、批量写入和流式查询功能。 |
