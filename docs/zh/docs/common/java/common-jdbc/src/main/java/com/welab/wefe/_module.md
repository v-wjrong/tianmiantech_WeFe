...
| 名称 | wefe |
| 编码语言 | .java |
| 代码路径 | WeFe/common/java/common-jdbc/src/main/java/com/welab/wefe |
| 包名 | docs.common.java.common-jdbc.src.main.java.com.welab.wefe |
...
该流程图展示了WeFe项目中common-jdbc模块的层级结构。从根目录wefe开始，逐级展开到common和jdbc目录，jdbc下包含hive、mysql和base三个子目录，每个子目录包含对应的Java文件。最外层jdbc目录还直接包含JdbcClient.java文件。整个结构清晰地反映了数据库相关组件的分类关系。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [common](common/_module.md) | package | HiveScanner和MysqlScanner是JdbcScanner的子类，分别用于Hive和MySQL查询。JdbcScanner是抽象基类，封装JDBC查询流程，支持多数据库适配。JdbcClient封装JDBC操作，支持多种数据库，提供连接管理、批量写入和流式查询功能。 |
