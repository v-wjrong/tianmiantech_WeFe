...
| 名称 | data |
| 编码语言 | .java |
| 代码路径 | WeFe/common/java/common-data-mysql/src/main/java/com/welab/wefe/common/data |
| 包名 | docs.common.java.common-data-mysql.src.main.java.com.welab.wefe.common.data |
...
该流程图展示了WeFe项目中MySQL数据模块的完整层级结构。根节点"data"下包含"mysql"主模块，其下细分9个子模块和3个独立文件。各子模块进一步展开具体实现类，如repository包含JPA仓库实现，entity包含三种抽象实体类。整体结构清晰体现了数据访问层的模块化设计，包含核心功能、工具类、监控和配置等完整组件。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [mysql](mysql/_module.md) | package | MyJpaRepository提供分页查询和字符串处理默认方法。PageUtils处理单对象分页。OrderBy枚举定义排序方向。CurdException为自定义异常。实体类继承体系管理通用字段。SQL监控模块捕获慢SQL和错误。抽象JPA配置类创建实体管理器工厂和数据源。MySpecification实现动态查询条件构建。DataMysql为数据库操作基础类。Where类链式构建查询条件。 |
