...
| 名称 | welab |
| 编码语言 | .java |
| 代码路径 | WeFe/common/java/common-wefe/src/main/java/com/welab |
| 包名 | docs.common.java.common-wefe.src.main.java.com.welab |
...
该流程图展示了WeFe项目中common-wefe模块的完整目录结构，从顶层wefe目录开始，逐级展开checkpoint、enums和dto三个核心子模块。其中enums包含40余个枚举类文件，dto模块下又细分为global_config和storage等子目录，global_config进一步包含calculation_engine、base等配置类。整个结构清晰呈现了项目中的类型定义、数据传输对象和检查点管理等核心组件，体现了模块化设计的层次关系。
