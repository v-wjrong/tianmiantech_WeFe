...
| 名称 | wefe |
| 编码语言 | .java |
| 代码路径 | WeFe/common/java/common-wefe/src/main/java/com/welab/wefe |
| 包名 | docs.common.java.common-wefe.src.main.java.com.welab.wefe |
...
该流程图展示了WeFe项目中common模块的详细目录结构，从根目录wefe开始，逐级展开到各个子目录和文件。重点呈现了checkpoint、enums和dto三大核心模块的层级关系，其中enums包含大量枚举类文件，dto下分global_config和storage等子模块，global_config又细分为calculation_engine、base等配置类。整体结构清晰反映了项目在代码组织上的模块化设计思想。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [common](common/_module.md) | package | 服务检查框架实现分层健康检测，枚举模块定义联邦学习类型状态，配置管理模块统一处理多源连接，数据类型推断器分析字段类型。 |
