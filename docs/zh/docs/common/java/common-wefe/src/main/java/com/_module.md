...
| 名称 | com |
| 编码语言 | .java |
| 代码路径 | WeFe/common/java/common-wefe/src/main/java/com |
| 包名 | docs.common.java.common-wefe.src.main.java.com |
...
该流程图展示了WeFe项目中common模块的Java代码结构，从顶层com包开始，逐级展开到welab、wefe层级。重点展示了common、dto、checkpoint和enums等核心模块的层级关系，其中enums模块包含大量枚举类文件，dto模块下包含global_config配置相关的子模块和文件。整体结构清晰展现了项目的基础设施和配置管理架构。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [welab](welab/_module.md) | package | 服务检查框架实现分层健康检测，枚举模块定义联邦学习类型状态，配置管理模块统一处理多源连接，数据类型推断器分析字段类型。 |
