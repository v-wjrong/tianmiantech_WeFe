...
| 名称 | data |
| 编码语言 | .java |
| 代码路径 | WeFe/common/java/common-data-storage/src/main/java/com/welab/wefe/common/data |
| 包名 | docs.common.java.common-data-storage.src.main.java.com.welab.wefe.common.data |
...
该流程图展示了WeFe数据存储模块的层级结构，从顶层data目录开始，逐级展开storage子目录及其各类服务组件。包含common基础类、fc文件存储服务、persistent持久化存储服务、backup备份模块和model数据模型。其中fc服务细分为阿里云和腾讯云实现，persistent服务支持ClickHouse和MySQL数据库存储，model包含分页和基础数据模型类。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [storage](storage/_module.md) | package | 模块1：定义数据存储通用配置，含序列化模式、中间件类型及数据库连接，依赖MySQL驱动。  模块2：统一多云平台与多数据库存储，支持批量操作、动态分片，依赖云SDK和JDBC。  模块3：提供分页查询基础模型，含分页输入输出及键值对结构，适用于日志等场景。 |
