# 基础信息

|      |      |
|------|------|
| 名称 | persistent |
| 编码语言 | .java |
| 代码路径 | WeFe/common/java/common-data-storage/src/main/java/com/welab/wefe/common/data/storage/service/persistent |
| 包名 | docs.common.java.common-data-storage.src.main.java.com.welab.wefe.common.data.storage.service.persistent |
| 概述说明 |  |

# 说明

None


### 包内部结构视图

None

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [mysql](mysql/_module.md) | package |  |
| [clickhouse](clickhouse/_module.md) | package |  |
| [PersistentStorage.java](PersistentStorage.md) | file |  |
| [PersistentStorageStreamHandler.java](PersistentStorageStreamHandler.md) | file |  |


