...
| 名称 | common |
| 编码语言 | .java |
| 代码路径 | WeFe/common/java/common-data-storage/src/main/java/com/welab/wefe/common |
| 包名 | docs.common.java.common-data-storage.src.main.java.com.welab.wefe.common |
...
该流程图展示了WeFe项目中common-data-storage模块的Java类文件层级结构。从根目录common开始，逐级展开data、storage等目录，最终显示各类Java文件。包含storage服务的fc和persistent子模块，以及model和backup等目录下的具体实现类，清晰呈现了模块间的从属关系。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [data](data/_module.md) | package | 模块1：定义数据存储通用配置，含序列化模式、中间件类型及数据库连接，依赖MySQL驱动。  模块2：统一多云平台与多数据库存储，支持批量操作、动态分片，依赖云SDK和JDBC。  模块3：提供分页查询基础模型，含分页输入输出及键值对结构，适用于日志等场景。 |
