...
| 名称 | wefe |
| 编码语言 | .java |
| 代码路径 | WeFe/common/java/common-data-mongodb/src/main/java/com/welab/wefe |
| 包名 | docs.common.java.common-data-mongodb.src.main.java.com.welab.wefe |
...
该流程图展示了WeFe项目中common-data-mongodb模块的完整目录结构，从顶层wefe包开始，逐级展开到最底层的具体Java文件。主要包含repo、constant、util、entity、dto和config六大核心模块，其中entity下又细分为common、manager、union和base四个子模块，每个模块都包含相应的业务实体类。repo模块包含大量MongoDB仓库接口，dto模块包含各类数据传输对象，整体结构清晰展现了MongoDB数据访问层的完整架构。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [common](common/_module.md) | package | MongoDB数据访问层提供CRUD和复杂查询功能，支持逻辑删除和时间戳约束。包含短信业务分类、供应商枚举管理。工具集支持链式查询构建和聚合操作。联邦学习平台实现日志、证书和数据治理。统一查询管理数据集和成员认证。连接管理模块动态注册MongoDB组件。 |
