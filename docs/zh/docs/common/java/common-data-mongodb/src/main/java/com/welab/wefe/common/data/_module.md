...
| 名称 | data |
| 编码语言 | .java |
| 代码路径 | WeFe/common/java/common-data-mongodb/src/main/java/com/welab/wefe/common/data |
| 包名 | docs.common.java.common-data-mongodb.src.main.java.com.welab.wefe.common.data |
...
该流程图展示了WeFe项目中common-data-mongodb模块的完整目录结构，从顶层data目录开始，逐级展开mongodb子目录及其包含的repo、constant、util、entity、dto和config等核心组件。实体类(entity)下细分了common、manager、union和base四个子模块，其中union模块包含大量业务实体和对应的扩展类(ext)。仓库层(repo)包含30余个MongoDB操作接口，工具类(util)提供查询构建功能，配置类(config)管理数据库连接。整个结构清晰体现了MongoDB数据访问层的模块化设计。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [mongodb](mongodb/_module.md) | package | MongoDB数据访问层提供CRUD和复杂查询功能，支持逻辑删除和时间戳约束。包含短信业务分类、供应商枚举管理。工具集支持链式查询构建和聚合操作。联邦学习平台实现日志、证书和数据治理。统一查询管理数据集和成员认证。连接管理模块动态注册MongoDB组件。 |
