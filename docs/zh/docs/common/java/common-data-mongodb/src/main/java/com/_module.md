...
| 名称 | com |
| 编码语言 | .java |
| 代码路径 | WeFe/common/java/common-data-mongodb/src/main/java/com |
| 包名 | docs.common.java.common-data-mongodb.src.main.java.com |
...
该流程图展示了WeFe项目中common-data-mongodb模块的完整Java包结构，从com.welab.wefe.common.data.mongodb根包开始，逐级展开至最底层的实体类、仓库接口和工具类。图中包含6个主要子模块（repo/constant/util/entity/dto/config），其中entity包下又细分出4个二级子包，完整呈现了MongoDB相关功能的代码组织结构。所有节点均使用路径末端名称，严格遵循了给定的56个路径条目。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [welab](welab/_module.md) | package | MongoDB数据访问层提供CRUD和复杂查询功能，支持逻辑删除和时间戳约束。包含短信业务分类、供应商枚举管理。工具集支持链式查询构建和聚合操作。联邦学习平台实现日志、证书和数据治理。统一查询管理数据集和成员认证。连接管理模块动态注册MongoDB组件。 |
