...
| 名称 | common |
| 编码语言 | .java |
| 代码路径 | WeFe/common/java/common-data-mongodb/src/main/java/com/welab/wefe/common |
| 包名 | docs.common.java.common-data-mongodb.src.main.java.com.welab.wefe.common |
...
该流程图展示了WeFe项目中common-data-mongodb模块的完整目录结构，从顶层common目录开始，逐级展开到具体的Java文件。主要包含repo仓库层、constant常量、util工具类、entity实体类、dto数据传输对象以及config配置等核心模块，其中entity下又细分为common、manager、union和base四个子模块，体现了清晰的分层架构设计。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [data](data/_module.md) | package | MongoDB数据访问层提供CRUD和复杂查询功能，支持逻辑删除和时间戳约束。包含短信业务分类、供应商枚举管理。工具集支持链式查询构建和聚合操作。联邦学习平台实现日志、证书和数据治理。统一查询管理数据集和成员认证。连接管理模块动态注册MongoDB组件。 |
