...
| 名称 | wefe |
| 编码语言 | .java |
| 代码路径 | WeFe/fusion/fusion-core/src/main/java/com/welab/wefe |
| 包名 | docs.fusion.fusion-core.src.main.java.com.welab.wefe |
...
该流程图展示了WeFe项目中fusion-core模块的Java代码结构，从根目录wefe开始，逐级展开到各个子模块和文件。核心模块包含执行器(actuator)、工具类(utils)、枚举(enums)和数据对象(dto)四大分支，其中PSI相关功能集中在actuator/psi目录下，工具类包含布隆过滤器和加密等实用工具，枚举类定义了系统使用的各种状态和类型，数据对象则封装了传输数据结构。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [fusion](fusion/_module.md) | package | 模块实现隐私保护集合交集(PSI)协议，含Server/Client基类、数据转换和元数据处理，依赖RSA-PSI算法和JObject序列化。布隆过滤器模块支持高效操作与持久化，集成密码学工具和线程池管理。枚举模块定义联邦学习关键维度如任务状态和算法类型。DTO模块封装布隆过滤器及元数据，支持PSI协议组件序列化传输。 |
