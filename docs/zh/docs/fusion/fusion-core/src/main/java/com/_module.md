...
| 名称 | com |
| 编码语言 | .java |
| 代码路径 | WeFe/fusion/fusion-core/src/main/java/com |
| 包名 | docs.fusion.fusion-core.src.main.java.com |
...
该流程图展示了WeFe融合核心模块的完整Java包结构，从根目录com开始逐级展开至最终实现文件。核心模块分为执行器(actuator)、工具类(utils)、枚举(enums)和数据对象(dto)四大分支，其中执行器下包含PSI协议相关实现，工具类包含布隆过滤器和加密工具，枚举定义了系统所有状态类型，数据对象则包含核心传输数据结构。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [welab](welab/_module.md) | package | 模块实现隐私保护集合交集(PSI)协议，含Server/Client基类、数据转换和元数据处理，依赖RSA-PSI算法和JObject序列化。布隆过滤器模块支持高效操作与持久化，集成密码学工具和线程池管理。枚举模块定义联邦学习关键维度如任务状态和算法类型。DTO模块封装布隆过滤器及元数据，支持PSI协议组件序列化传输。 |
