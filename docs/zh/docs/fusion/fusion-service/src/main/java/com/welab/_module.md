...
| 名称 | welab |
| 编码语言 | .java |
| 代码路径 | WeFe/fusion/fusion-service/src/main/java/com/welab |
| 包名 | docs.fusion.fusion-service.src.main.java.com.welab |
...
该流程图展示了WeFe/fusion项目的完整目录结构，从顶层wefe节点开始，逐级展开至最底层的具体文件。主要包含api、service、database、utils等核心模块，其中api模块下细分了system、account、partner等多个子模块，每个子模块包含对应的接口文件；service模块则包含了业务逻辑处理相关的服务类。整个结构层次清晰，完整呈现了项目各模块间的从属关系。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [wefe](wefe/_module.md) | package | 联邦学习平台核心系统，含控制、数据、任务模块，支持初始化、数据管理、任务调度全流程。分层架构设计，统一接口规范，关键数据结构和多服务组件集成。 |
