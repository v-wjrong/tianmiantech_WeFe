...
| 名称 | fusion |
| 编码语言 | .java |
| 代码路径 | WeFe/fusion/fusion-service/src/main/java/com/welab/wefe/data/fusion |
| 包名 | docs.fusion.fusion-service.src.main.java.com.welab.wefe.data.fusion |
...
该流程图展示了WeFe数据融合服务的完整项目结构，从顶层fusion目录开始，逐级展开所有子模块和文件。主要包含service、api、database、utils等核心模块，其中api模块下细分了system、account、partner等多个子功能接口，database模块包含repository和entity等数据库相关组件，utils模块提供各类工具类支持。整体结构层次清晰，模块划分明确，体现了典型的企业级Java服务端项目架构。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [service](service/_module.md) | package | 联邦学习平台核心系统，含控制、数据、任务模块，支持初始化、数据管理、任务调度全流程。分层架构设计，统一接口规范，关键数据结构和多服务组件集成。 |
