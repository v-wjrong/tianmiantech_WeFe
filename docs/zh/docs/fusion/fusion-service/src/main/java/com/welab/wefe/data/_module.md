...
| 名称 | data |
| 编码语言 | .java |
| 代码路径 | WeFe/fusion/fusion-service/src/main/java/com/welab/wefe/data |
| 包名 | docs.fusion.fusion-service.src.main.java.com.welab.wefe.data |
...
该流程图展示了WeFe数据融合服务的完整目录结构，从顶层data目录开始逐级展开，包含api接口、数据库操作、工具类、枚举定义、服务实现等12个主要模块。其中api模块下细分8个子模块包含47个接口文件，database模块包含完整的实体类和仓储层实现，service模块实现核心业务逻辑。整体结构清晰体现了分层架构设计，各模块职责分明，便于维护和扩展。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [fusion](fusion/_module.md) | package | 联邦学习平台核心系统，含控制、数据、任务模块，支持初始化、数据管理、任务调度全流程。分层架构设计，统一接口规范，关键数据结构和多服务组件集成。 |
