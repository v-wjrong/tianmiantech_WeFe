# 基础信息

|      |      |
|------|------|
| 名称 | task |
| 编码语言 | .java |
| 代码路径 | WeFe/fusion/fusion-service/src/main/java/com/welab/wefe/data/fusion/service/task |
| 包名 | docs.fusion.fusion-service.src.main.java.com.welab.wefe.data.fusion.service.task |
| 概述说明 |  |

# 说明

None


### 包内部结构视图

None

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [AbstractTask.java](AbstractTask.md) | file |  |
| [PsiServerTask.java](PsiServerTask.md) | file |  |
| [AbstractPsiTask.java](AbstractPsiTask.md) | file |  |
| [PsiClientTask.java](PsiClientTask.md) | file |  |


