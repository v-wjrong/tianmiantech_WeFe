# 基础信息

|      |      |
|------|------|
| 名称 | Config |
| 编码语言 | .java |
| 代码路径 | WeFe/fusion/fusion-service/src/main/java/com/welab/wefe/data/fusion/service/config/Config.java |
| 包名 | com.welab.wefe.data.fusion.service.config |
| 依赖项 | ['com.welab.wefe.common.web.config.CommonConfig', 'org.springframework.beans.factory.annotation.Value', 'org.springframework.boot.context.properties.ConfigurationProperties', 'org.springframework.context.annotation.PropertySource', 'org.springframework.stereotype.Component', 'java.nio.file.Paths'] |
| 概述说明 |  |

# 说明



# 类列表 Class Summary

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| Config | class |  |



## 类 Config

|      |      |
|------|------|
| 访问范围 | @Component;@PropertySource(value = {"file:${config.path}"}, encoding = "utf-8");@ConfigurationProperties;public |
| 类型 | class |
| 名称 | Config |
| 说明 |  |


### UML类图




### 内部方法调用关系图



### 字段列表 Field List

| 名称  | 类型  | 说明 |
|-------|-------|------|
| verificationCodeSendChannel | String |  |
| smsAccessKeyId | String |  |
| smsAliyunAccountForgetPasswordVerificationCodeTemplateCode | String |  |
| socketServerIp | String |  |
| smsAccessKeySecret | String |  |
| smsAliyunMemberregisterVerificationCodeTemplateCode | String |  |
| smsAliyunSignName | String |  |

### 方法列表

| 名称  | 类型  | 说明 |
|-------|-------|------|
| getSmsAliyunMemberregisterVerificationCodeTemplateCode | String |  |
| setSmsAccessKeySecret | void |  |
| getSmsAliyunSignName | String |  |
| getSmsAliyunAccountForgetPasswordVerificationCodeTemplateCode | String |  |
| getBloomFilterDir | String |  |
| setVerificationCodeSendChannel | void |  |
| setSmsAccessKeyId | void |  |
| getVerificationCodeSendChannel | String |  |
| getSmsAccessKeyId | String |  |
| getSmsAccessKeySecret | String |  |
| setSmsAliyunMemberregisterVerificationCodeTemplateCode | void |  |
| setSmsAliyunAccountForgetPasswordVerificationCodeTemplateCode | void |  |
| getSourceFilterDir | String |  |
| getSocketServerIp | String |  |
| setSocketServerIp | void |  |
| setSmsAliyunSignName | void |  |




