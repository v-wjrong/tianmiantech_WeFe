...
| 名称 | com |
| 编码语言 | .java |
| 代码路径 | WeFe/fusion/fusion-service/src/main/java/com |
| 包名 | docs.fusion.fusion-service.src.main.java.com |
...
该流程图展示了WeFe/fusion/fusion-service项目的完整Java包结构，从顶层service节点开始，逐级展开api、repo、database等主要模块。api模块包含system、account等子模块及具体接口类；database模块包含repository和entity等数据持久层结构；service模块则实现了业务逻辑层。整个结构层次清晰，模块间关系明确，体现了典型的企业级Java应用分层架构设计。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [welab](welab/_module.md) | package | 联邦学习平台核心系统，含控制、数据、任务模块，支持初始化、数据管理、任务调度全流程。分层架构设计，统一接口规范，关键数据结构和多服务组件集成。 |
