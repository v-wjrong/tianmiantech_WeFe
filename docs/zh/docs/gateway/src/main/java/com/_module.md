...
| 名称 | com |
| 编码语言 | .java |
| 代码路径 | WeFe/gateway/src/main/java/com |
| 包名 | docs.gateway.src.main.java.com |
...
该流程图展示了WeFe网关项目的完整目录结构，从顶层gateway节点开始，逐级展开到各个子模块和文件。主要包含14个一级模块：初始化配置(init)、拦截器(interceptor)、API接口(api)、数据存储(repository)、SDK工具(sdk)、实用工具(util)、通用组件(common)、服务层(service)、实体类(entity)、定时任务(scheduler)、缓存管理(cache)、监听器(listener)、基础组件(base)和配置管理(config)。其中service模块进一步细分为处理器(processors)和基础服务(base)两大子模块，processors又包含可用性检查(available)等具体实现。整个结构清晰体现了微服务架构的分层设计思想。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [welab](welab/_module.md) | package | gRPC网关系统，含服务器管理、缓存加载、安全拦截、数据传输、存储访问、定时刷新、健康检查等功能模块，支持双通道通信、动态TLS、流式处理及分布式监控。 |
