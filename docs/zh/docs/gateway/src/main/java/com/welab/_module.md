...
| 名称 | welab |
| 编码语言 | .java |
| 代码路径 | WeFe/gateway/src/main/java/com/welab |
| 包名 | docs.gateway.src.main.java.com.welab |
...
该流程图展示了WeFe网关项目的完整目录结构，从顶层wefe节点开始，逐级展开到gateway及其所有子模块。图中清晰呈现了12个主要模块（如init、interceptor、service等）及其下属文件，特别是service模块下细分的processors和base子模块的复杂结构。所有节点均使用路径末端名称，严格遵循了路径层级关系，完整覆盖了给定的全部路径信息。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [wefe](wefe/_module.md) | package | gRPC网关系统，含服务器管理、缓存加载、安全拦截、数据传输、存储访问、定时刷新、健康检查等功能模块，支持双通道通信、动态TLS、流式处理及分布式监控。 |
