...
| 名称 | wefe |
| 编码语言 | .java |
| 代码路径 | WeFe/union/union-service/src/main/java/com/welab/wefe |
| 包名 | docs.union.union-service.src.main.java.com.welab.wefe |
...
该流程图展示了WeFe项目中union-service模块的完整目录结构，从顶层wefe目录开始，逐级展开到具体的Java文件。主要包含service、api、dto、contract等核心模块，其中api模块下细分了cert、member、dataresource等多个子模块，每个子模块包含具体的API实现类。整体结构清晰展示了模块间的层级关系和依赖关系，便于理解项目架构。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [union](union/_module.md) | package | 联盟服务模块群提供多维度管理能力，包括会员生命周期、数据资源操作、公共服务及监控。采用RESTful接口，统一继承AbstractApi基类，关键结构含MemberOutput等。支持四大场景：会员管理、数据CRUD、公共服务及健康检测。依赖MemberService等组件，集成智能合约与区块链技术，实现联邦学习数据协同与权限控制。 |
