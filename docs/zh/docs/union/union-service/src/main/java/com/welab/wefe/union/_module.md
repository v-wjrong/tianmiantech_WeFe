...
| 名称 | union |
| 编码语言 | .java |
| 代码路径 | WeFe/union/union-service/src/main/java/com/welab/wefe/union |
| 包名 | docs.union.union-service.src.main.java.com.welab.wefe.union |
...
该流程图展示了WeFe/union项目的完整目录结构，从顶层union目录开始，逐级展开service、dto、api等主要模块。其中api模块包含cert、member、dataresource等多个子模块，每个子模块下又有具体的API实现类。service模块包含各种服务类和合约服务实现，dto模块包含各类数据传输对象。整体结构清晰展示了项目的分层架构和模块间关系，包含超过100个节点但保持了良好的可读性。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [service](service/_module.md) | package | 联盟服务模块群提供多维度管理能力，包括会员生命周期、数据资源操作、公共服务及监控。采用RESTful接口，统一继承AbstractApi基类，关键结构含MemberOutput等。支持四大场景：会员管理、数据CRUD、公共服务及健康检测。依赖MemberService等组件，集成智能合约与区块链技术，实现联邦学习数据协同与权限控制。 |
