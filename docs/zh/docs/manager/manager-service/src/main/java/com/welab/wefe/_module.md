...
| 名称 | wefe |
| 编码语言 | .java |
| 代码路径 | WeFe/manager/manager-service/src/main/java/com/welab/wefe |
| 包名 | docs.manager.manager-service.src.main.java.com.welab.wefe |
...
该流程图展示了WeFe管理服务的完整目录结构，从顶层wefe节点开始，逐级展开到具体的Java文件。主要包含api、contract、service、dto等核心模块，其中api模块下细分了cert、account等12个子模块，每个子模块包含多个API接口文件；contract模块包含11个合约类；service模块包含13个服务类。整体结构清晰展现了后端服务的分层设计和功能划分，共涉及200+个文件节点，完整呈现了微服务架构的模块化组织方式。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [manager](manager/_module.md) | package | 联盟链资源管理模块群，提供证书、数据、节点等全生命周期管理，采用RESTful接口和MongoDB+智能合约实现数据同步。支持成员注册、数据上传、权限设置等业务闭环，含文件检查、版本生成、对象映射转换等功能。依赖Spring、MongoDB、FISCO BCOS SDK，确保事务一致性与安全加密。 |
