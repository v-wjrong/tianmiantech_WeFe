...
| 名称 | cert |
| 编码语言 | .java |
| 代码路径 | WeFe/manager/manager-service/src/main/java/com/webank/cert |
| 包名 | docs.manager.manager-service.src.main.java.com.webank.cert |
...
该流程图展示了WeFe项目中manager-service模块的Java包结构。从根目录cert开始，逐级展开mgr及其子目录（utils、enums、exception等），最终显示到具体的Java文件。每个节点仅显示最后一级名称，完整呈现了证书管理服务的代码组织结构，包含工具类、枚举、异常处理、服务层、模型、数据库访问等模块。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [mgr](mgr/_module.md) | package | TransformUtils提供对象转换功能，含三个静态常量和三个转换方法。CertMgrException是自定义异常类，含异常代码和描述。CertOperationService提供证书管理功能，包括更新状态、导出证书等。CertDao管理证书数据访问操作。CertBeans是Spring配置类，注册CertService。CertApp为空实现类，可能用于证书管理。模块实现数字证书全生命周期管理，涵盖申请、签发和密钥关联流程。 |
