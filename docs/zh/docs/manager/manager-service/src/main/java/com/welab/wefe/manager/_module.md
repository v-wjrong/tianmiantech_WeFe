...
| 名称 | manager |
| 编码语言 | .java |
| 代码路径 | WeFe/manager/manager-service/src/main/java/com/welab/wefe/manager |
| 包名 | docs.manager.manager-service.src.main.java.com.welab.wefe.manager |
...
该流程图展示了WeFe Manager服务的完整目录结构，从顶层manager节点开始，逐级展开service、api、contract等主要模块。api模块下包含cert、account等10个子模块，每个子模块包含具体的API实现类。contract模块包含12个合约类，service模块包含13个服务类。整体结构清晰展现了Java项目的标准分层架构，包含API层、服务层、数据访问层和DTO对象层等核心组件。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [service](service/_module.md) | package | 联盟链资源管理模块群，提供证书、数据、节点等全生命周期管理，采用RESTful接口和MongoDB+智能合约实现数据同步。支持成员注册、数据上传、权限设置等业务闭环，含文件检查、版本生成、对象映射转换等功能。依赖Spring、MongoDB、FISCO BCOS SDK，确保事务一致性与安全加密。 |
