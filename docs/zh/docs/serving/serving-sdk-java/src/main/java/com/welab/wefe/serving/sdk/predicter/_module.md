# 基础信息

|      |      |
|------|------|
| 名称 | predicter |
| 编码语言 | .java |
| 代码路径 | WeFe/serving/serving-sdk-java/src/main/java/com/welab/wefe/serving/sdk/predicter |
| 包名 | docs.serving.serving-sdk-java.src.main.java.com.welab.wefe.serving.sdk.predicter |
| 概述说明 |  |

# 说明

None


### 包内部结构视图

None

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [batch](batch/_module.md) | package |  |
| [single](single/_module.md) | package |  |
| [PredictBehavior.java](PredictBehavior.md) | file |  |
| [AbstractBasePredictor.java](AbstractBasePredictor.md) | file |  |


