...
| 名称 | welab |
| 编码语言 | .java |
| 代码路径 | WeFe/mpc/mpc-sa/mpc-sa-sdk/src/main/java/com/welab |
| 包名 | docs.mpc.mpc-sa.mpc-sa-sdk.src.main.java.com.welab |
...
该流程图展示了WeFe项目中MPC安全聚合SDK的层级结构。从welab根目录开始，逐级展开到具体的实现类和配置文件，包括传输变量实现、安全聚合主类和服务配置等核心组件。整个结构呈现清晰的树状依赖关系，深度为7层，共包含10个节点。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [wefe](wefe/_module.md) | package | 该模块通过HTTP协议实现安全数据传输，支持Diffie-Hellman密钥交换和结果查询，适用于联邦学习等场景。核心接口包括密钥协商和结果获取，依赖HTTP服务器配置。 |
