...
| 名称 | server |
| 编码语言 | .java |
| 代码路径 | WeFe/mpc/mpc-sa/mpc-sa-server/src/main/java/com/welab/wefe/mpc/sa/server |
| 包名 | docs.mpc.mpc-sa.mpc-sa-server.src.main.java.com.welab.wefe.mpc.sa.server |
...
该流程图展示了MPC-SA服务模块的层级结构，根节点为server目录，包含service子目录，service下包含两个服务类文件：QueryResultService和QueryDiffieHellmanKeyService。这种结构体现了典型的Java服务层代码组织方式，服务类被统一放置在service包中便于管理。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [service](service/_module.md) | package | QueryResultService处理查询请求，提供两种handle方法，涉及缓存获取、加密计算和结果调整，返回处理结果和UUID。QueryDiffieHellmanKeyService处理密钥交换请求，生成随机密钥，进行加密计算并缓存，返回加密结果和UUID。 |
