...
| 名称 | wefe |
| 编码语言 | .java |
| 代码路径 | WeFe/mpc/mpc-sa/mpc-sa-server/src/main/java/com/welab/wefe |
| 包名 | docs.mpc.mpc-sa.mpc-sa-server.src.main.java.com.welab.wefe |
...
该流程图展示了WeFe项目中MPC安全计算模块的服务端代码层级结构。从根目录wefe开始，依次经过mpc、sa、server层级，最终到达service层，该层包含两个服务类文件：QueryResultService和QueryDiffieHellmanKeyService。整个结构呈现清晰的树形依赖关系，共涉及7个路径节点。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [mpc](mpc/_module.md) | package | QueryResultService处理查询请求，提供两种handle方法，涉及缓存获取、加密计算和结果调整，返回处理结果和UUID。QueryDiffieHellmanKeyService处理密钥交换请求，生成随机密钥，进行加密计算并缓存，返回加密结果和UUID。 |
