...
| 名称 | psi |
| 编码语言 | .java |
| 代码路径 | WeFe/mpc/mpc-psi/mpc-psi-sdk/src/main/java/com/welab/wefe/mpc/psi |
| 包名 | docs.mpc.mpc-psi.mpc-psi-sdk.src.main.java.com.welab.wefe.mpc.psi |
...
该流程图展示了MPC-PSI-SDK项目的核心结构，以psi为根节点，向下延伸出sdk主模块。sdk模块包含多个子模块：ecdh加密模块、util工具集、service服务层、excel数据处理、pir隐私信息检索、dh密钥交换以及model数据模型。每个子模块下又包含具体的实现类文件，形成清晰的层级关系，完整呈现了该项目的主要组件和文件组织结构。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [sdk](sdk/_module.md) | package | 该SDK实现多方隐私集合求交(PSI)功能，支持ECDH和DH加密协议，包含服务端/客户端组件、数据预处理工具和工厂模式。核心流程为密钥生成、数据加密和结果比对，通过多线程优化性能，适用于安全数据匹配场景。 |
