...
| 名称 | welab |
| 编码语言 | .java |
| 代码路径 | WeFe/mpc/mpc-psi/mpc-psi-sdk/src/main/java/com/welab |
| 包名 | docs.mpc.mpc-psi.mpc-psi-sdk.src.main.java.com.welab |
...
该流程图展示了WeFe项目中MPC-PSI-SDK模块的完整目录结构，从根目录wefe开始，逐级展开到mpc、psi和sdk目录。sdk目录下包含多个子模块，如ecdh、util、service等，每个子模块又包含具体的实现类文件。整个结构清晰地呈现了PSI（Private Set Intersection）SDK的组件分布，包括核心类、工具类、服务接口以及不同协议（ECDH/DH）的实现。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [wefe](wefe/_module.md) | package | 该SDK实现多方隐私集合求交(PSI)功能，支持ECDH和DH加密协议，包含服务端/客户端组件、数据预处理工具和工厂模式。核心流程为密钥生成、数据加密和结果比对，通过多线程优化性能，适用于安全数据匹配场景。 |
