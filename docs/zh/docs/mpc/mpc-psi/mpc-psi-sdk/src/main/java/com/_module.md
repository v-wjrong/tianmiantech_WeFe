...
| 名称 | com |
| 编码语言 | .java |
| 代码路径 | WeFe/mpc/mpc-psi/mpc-psi-sdk/src/main/java/com |
| 包名 | docs.mpc.mpc-psi.mpc-psi-sdk.src.main.java.com |
...
该流程图展示了WeFe项目中MPC-PSI-SDK模块的Java代码层级结构。从根节点com开始，逐级展开到welab、wefe、mpc、psi，最终到sdk目录。sdk目录下包含多个子模块（如ecdh、util、service等）和核心类文件（如Psi.java）。每个子模块又包含具体的实现类，例如ecdh模块包含椭圆曲线相关类，util模块包含工具类。整体结构清晰展示了SDK的功能模块划分和依赖关系。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [welab](welab/_module.md) | package | 该SDK实现多方隐私集合求交(PSI)功能，支持ECDH和DH加密协议，包含服务端/客户端组件、数据预处理工具和工厂模式。核心流程为密钥生成、数据加密和结果比对，通过多线程优化性能，适用于安全数据匹配场景。 |
