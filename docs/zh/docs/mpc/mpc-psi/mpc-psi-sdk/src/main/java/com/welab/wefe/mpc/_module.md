...
| 名称 | mpc |
| 编码语言 | .java |
| 代码路径 | WeFe/mpc/mpc-psi/mpc-psi-sdk/src/main/java/com/welab/wefe/mpc |
| 包名 | docs.mpc.mpc-psi.mpc-psi-sdk.src.main.java.com.welab.wefe.mpc |
...
该流程图展示了WeFe MPC PSI SDK的完整层级结构，从根目录mpc开始，逐级展开到psi、sdk等子目录，最终显示所有关键实现类文件。图中清晰呈现了ecdh加密模块、工具类(util)、服务接口(service)、Excel处理模块(excel)等核心组件及其关联关系，共包含25个节点，完整覆盖了PSI SDK的功能模块划分。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [psi](psi/_module.md) | package | 该SDK实现多方隐私集合求交(PSI)功能，支持ECDH和DH加密协议，包含服务端/客户端组件、数据预处理工具和工厂模式。核心流程为密钥生成、数据加密和结果比对，通过多线程优化性能，适用于安全数据匹配场景。 |
