...
| 名称 | welab |
| 编码语言 | .java |
| 代码路径 | WeFe/mpc/mpc-common/src/main/java/com/welab |
| 包名 | docs.mpc.mpc-common.src.main.java.com.welab |
...
该流程图展示了WeFe/mpc/mpc-common项目的完整目录结构，从mpc根目录开始，逐级展开所有子模块和文件。重点展示了trasfer、sa、util、commom、psi、cache、key、pir和config等主要模块及其下属文件，特别是pir模块下复杂的protocol子结构，包含ro、nt、se、ot等多个加密协议实现层。整个结构清晰体现了MPC(安全多方计算)项目的模块化设计，特别是密码学相关组件的分层架构。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [wefe](wefe/_module.md) | package | AbstractHttpTransferVariable是处理HTTP请求的抽象类，支持JSON转换和签名验证。安全聚合模块实现密钥交换和加密通信。密码学工具集提供SM2/RSA等算法。MPC基础工具集支持数值转换和随机数据生成。PSI模块处理私有集合交集查询。本地缓存系统采用两级结构。DiffieHellmanKey类管理DH协议参数。PIR模块实现隐私信息检索协议。通信配置类定义请求参数。 |
