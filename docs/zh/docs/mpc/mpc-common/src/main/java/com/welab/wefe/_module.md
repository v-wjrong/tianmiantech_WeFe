...
| 名称 | wefe |
| 编码语言 | .java |
| 代码路径 | WeFe/mpc/mpc-common/src/main/java/com/welab/wefe |
| 包名 | docs.mpc.mpc-common.src.main.java.com.welab.wefe |
...
该流程图展示了WeFe项目中mpc模块的完整目录结构，从根目录wefe开始，逐级展开到各个子模块和文件。重点展示了mpc-common模块下的核心功能组件，包括安全聚合(sa)、隐私信息检索(pir)、缓存系统(cache)、密钥管理(key)等模块的层级关系。图中详细呈现了每个模块内部的类文件分布，特别是pir协议层复杂的加密算法实现结构，包括RO/NT/SE/OT等多种协议实现细节。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [mpc](mpc/_module.md) | package | AbstractHttpTransferVariable是处理HTTP请求的抽象类，支持JSON转换和签名验证。安全聚合模块实现密钥交换和加密通信。密码学工具集提供SM2/RSA等算法。MPC基础工具集支持数值转换和随机数据生成。PSI模块处理私有集合交集查询。本地缓存系统采用两级结构。DiffieHellmanKey类管理DH协议参数。PIR模块实现隐私信息检索协议。通信配置类定义请求参数。 |
