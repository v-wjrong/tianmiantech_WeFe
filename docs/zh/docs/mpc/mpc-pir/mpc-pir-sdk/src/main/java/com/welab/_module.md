...
| 名称 | welab |
| 编码语言 | .java |
| 代码路径 | WeFe/mpc/mpc-pir/mpc-pir-sdk/src/main/java/com/welab |
| 包名 | docs.mpc.mpc-pir.mpc-pir-sdk.src.main.java.com.welab |
...
该流程图展示了WeFe项目中MPC-PIR-SDK模块的完整层级结构，从根目录wefe开始，逐级展开到最底层的实现文件和配置文件。图中清晰呈现了trasfer、naor、query等核心功能模块的从属关系，以及各模块下的具体实现类如HttpTransferVariable.java和PrivateInformationRetrievalClient.java等。整个结构共包含17个节点，完整覆盖了给定的所有路径信息，准确反映了SDK内部的组织架构。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [wefe](wefe/_module.md) | package | 该模块实现基于Naor-Pinkas协议的安全私有信息检索，包含数据混淆、OT交互和加密查询功能。核心类处理密钥生成、参数验证和结果解密，支持客户端-服务器安全检索流程。提供混淆接口生成差异化实例，配置类校验参数合法性。整体确保查询隐私性和流程安全性。 |
