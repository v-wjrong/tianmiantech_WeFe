...
| 名称 | pir |
| 编码语言 | .java |
| 代码路径 | WeFe/mpc/mpc-pir/mpc-pir-server/src/main/java/com/welab/wefe/mpc/pir |
| 包名 | docs.mpc.mpc-pir.mpc-pir-server.src.main.java.com.welab.wefe.mpc.pir |
...
该流程图展示了MPC-PIR服务器项目的核心结构，以pir为根节点，包含server主模块及其8个子模块。server模块下包含事件处理、数据传输、线程管理、协议实现、服务层、缓存管理、流程控制等组件，每个子模块又包含具体的实现类文件，形成完整的服务器架构体系。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [server](server/_module.md) | package | PrivateInformationRetrievalEvent类处理私有信息检索事件，含uuid和keys属性。模块实现PIR协议数据传输与验证，管理随机数和加密数据。ProduceHauckTargetThread线程生成并缓存HauckTarget对象。HauckObliviousTransferSender类实现密钥生成流程。模块基于Naor-Pinkas协议提供安全查询服务。HauckTargetCache单例类管理线程安全缓存。PrivateInformationRetrievalFlowServer类处理检索流程。PrivateInformationRetrievalServer类提供服务器初始化和缓存管理功能。 |
