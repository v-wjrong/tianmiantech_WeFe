...
| 名称 | mpc |
| 编码语言 | .java |
| 代码路径 | WeFe/mpc/mpc-pir/mpc-pir-server/src/main/java/com/welab/wefe/mpc |
| 包名 | docs.mpc.mpc-pir.mpc-pir-server.src.main.java.com.welab.wefe.mpc |
...
该流程图展示了MPC-PIR服务器模块的完整结构，从根目录mpc开始，逐级展开到pir、server等子目录，最终显示所有关键组件文件。server作为核心节点，包含8个主要功能模块(event、trasfer、thread等)，每个模块下又细分具体实现类，如event包含事件处理类，service包含多种加密服务类，完整呈现了私有信息检索服务器的架构体系。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [pir](pir/_module.md) | package | PrivateInformationRetrievalEvent类处理私有信息检索事件，含uuid和keys属性。模块实现PIR协议数据传输与验证，管理随机数和加密数据。ProduceHauckTargetThread线程生成并缓存HauckTarget对象。HauckObliviousTransferSender类实现密钥生成流程。模块基于Naor-Pinkas协议提供安全查询服务。HauckTargetCache单例类管理线程安全缓存。PrivateInformationRetrievalFlowServer类处理检索流程。PrivateInformationRetrievalServer类提供服务器初始化和缓存管理功能。 |
