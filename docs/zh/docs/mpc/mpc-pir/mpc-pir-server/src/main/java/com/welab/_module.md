...
| 名称 | welab |
| 编码语言 | .java |
| 代码路径 | WeFe/mpc/mpc-pir/mpc-pir-server/src/main/java/com/welab |
| 包名 | docs.mpc.mpc-pir.mpc-pir-server.src.main.java.com.welab |
...
该流程图展示了WeFe项目中MPC-PIR服务器的完整目录结构，从根目录wefe开始，逐级展开到具体的实现类和接口。重点展示了server目录下的8个子模块及其关联关系，包括事件处理、传输协议、线程管理、服务实现等核心组件，最终形成完整的私有信息检索服务器架构。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [wefe](wefe/_module.md) | package | PrivateInformationRetrievalEvent类处理私有信息检索事件，含uuid和keys属性。模块实现PIR协议数据传输与验证，管理随机数和加密数据。ProduceHauckTargetThread线程生成并缓存HauckTarget对象。HauckObliviousTransferSender类实现密钥生成流程。模块基于Naor-Pinkas协议提供安全查询服务。HauckTargetCache单例类管理线程安全缓存。PrivateInformationRetrievalFlowServer类处理检索流程。PrivateInformationRetrievalServer类提供服务器初始化和缓存管理功能。 |
