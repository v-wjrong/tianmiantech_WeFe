...
| Name | welab |
| Language | .java |
| Code Path | WeFe/mpc/mpc-sa/mpc-sa-server/src/main/java/com/welab |
| Package Name | docs.mpc.mpc-sa.mpc-sa-server.src.main.java.com.welab |
...
This flowchart illustrates the complete hierarchical relationship from welab to the final service files, comprising a total of 7 nodes. The path starts from welab and progressively delves into wefe, mpc, sa, server, and service directories, ultimately branching into two service class files: QueryResultService and QueryDiffieHellmanKeyService. The entire structure clearly demonstrates the organization of Java package paths.

# File List

| Name   | Type  | Description |
|-------|------|-------------|
| [wefe](wefe/_module.md) | package | The QueryResultService processes query requests, providing two handle methods that involve cache retrieval, encryption computation, and result adjustment, returning the processed result and UUID. The QueryDiffieHellmanKeyService handles key exchange requests, generates random keys, performs encryption computation and caching, and returns the encrypted result and UUID. |
