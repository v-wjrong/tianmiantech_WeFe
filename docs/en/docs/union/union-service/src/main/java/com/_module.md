...
| Name | com |
| Language | .java |
| Code Path | WeFe/union/union-service/src/main/java/com |
| Package Name | docs.union.union-service.src.main.java.com |
...
This flowchart presents the complete directory structure of the WeFe/union/union-service project, starting from the top-level union node and hierarchically expanding into major modules such as service, api, and dto. The api module includes submodules like cert, member, and dataresource, each containing specific API implementation classes. The service module comprises various service classes and contract service implementations, while the dto module includes various data transfer objects. The entire structure is clearly layered, fully demonstrating the project's module division and file organization.

# File List

| Name   | Type  | Description |
|-------|------|-------------|
| [welab](welab/_module.md) | package | The Alliance Service Module Cluster provides multi-dimensional management capabilities, including member lifecycle, data resource operations, public services, and monitoring. It adopts RESTful interfaces, uniformly inherits the AbstractApi base class, and features key structures such as MemberOutput. It supports four major scenarios: member management, data CRUD, public services, and health checks. Relying on components like MemberService, it integrates smart contracts and blockchain technology to enable federated learning data collaboration and permission control. |
