...
| Name | com |
| Language | .java |
| Code Path | WeFe/gateway/src/main/java/com |
| Package Name | docs.gateway.src.main.java.com |
...
This flowchart presents the complete directory structure of the WeFe Gateway project, starting from the top-level gateway node and hierarchically expanding to various submodules and files. It primarily consists of 14 first-level modules: initialization configuration (init), interceptors (interceptor), API interfaces (api), data storage (repository), SDK utilities (sdk), utility tools (util), common components (common), service layer (service), entity classes (entity), scheduled tasks (scheduler), cache management (cache), listeners (listener), foundational components (base), and configuration management (config). The service module is further subdivided into two major submodules: processors and base services, with processors including specific implementations such as availability checks (available). The entire structure clearly reflects the layered design philosophy of microservice architecture.

# File List

| Name   | Type  | Description |
|-------|------|-------------|
| [welab](welab/_module.md) | package | A gRPC gateway system comprising functional modules such as server management, cache loading, security interception, data transmission, storage access, scheduled refresh, and health checks, supporting dual-channel communication, dynamic TLS, stream processing, and distributed monitoring. |
