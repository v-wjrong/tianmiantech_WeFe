...
| Name | fusion |
| Language | .java |
| Code Path | WeFe/fusion/fusion-service/src/main/java/com/welab/wefe/data/fusion |
| Package Name | docs.fusion.fusion-service.src.main.java.com.welab.wefe.data.fusion |
...
This flowchart illustrates the complete project structure of the WeFe data fusion service, starting from the top-level fusion directory and hierarchically expanding all submodules and files. It primarily includes core modules such as service, api, database, and utils. The api module is further subdivided into multiple functional interfaces including system, account, and partner. The database module contains database-related components like repository and entity, while the utils module provides various utility class support. The overall structure is clearly layered with well-defined module divisions, reflecting a typical enterprise-level Java server-side project architecture.

# File List

| Name   | Type  | Description |
|-------|------|-------------|
| [service](service/_module.md) | package | The core system of the federated learning platform includes control, data, and task modules, supporting the entire workflow from initialization to data management and task scheduling. It features a layered architecture design, unified interface standards, and integrates key data structures with multiple service components. |
