...
| Name | data |
| Language | .java |
| Code Path | WeFe/fusion/fusion-service/src/main/java/com/welab/wefe/data |
| Package Name | docs.fusion.fusion-service.src.main.java.com.welab.wefe.data |
| Brief Description | Federated Learning Platform Core System, including control, data, and task modules, supports the entire workflow from initialization, data management to task scheduling. Features a layered architecture design, unified interface standards, and integrates key data structures with multiple service components. |
...
This flowchart presents the complete directory structure of the WeFe data fusion service, starting from the top-level data directory and hierarchically expanding to include 12 main modules such as API interfaces, database operations, utility classes, enumeration definitions, and service implementations. The API module is subdivided into 8 submodules containing 47 interface files, the database module encompasses complete entity classes and repository layer implementations, while the service module handles core business logic. The overall structure clearly reflects a layered architecture design with well-defined responsibilities for each module, facilitating maintenance and extensibility.

# File List

| Name   | Type  | Description |
|-------|------|-------------|
| [fusion](fusion/_module.md) | package | The core system of the federated learning platform includes control, data, and task modules, supporting the entire workflow of initialization, data management, and task scheduling. It features a layered architecture design, unified interface standards, and integrates key data structures with multiple service components. |
