...
| Name | com |
| Language | .java |
| Code Path | WeFe/serving/serving-sdk-java/src/main/java/com |
| Package Name | docs.serving.serving-sdk-java.src.main.java.com |
...
This flowchart presents the complete directory structure of the WeFe service SDK, starting from the top-level serving node and expanding into 10 main modules under the SDK. Each module contains specific submodules and implementation classes, with the algorithm module particularly detailing the single-machine and batch processing implementations for both lr and xgboost algorithms. The model module displays the corresponding model structures for different algorithms. The entire structure is clearly hierarchical, fully demonstrating the functional module division of the machine learning service SDK.

# File List

| Name   | Type  | Description |
|-------|------|-------------|
| [welab](welab/_module.md) | package | This module provides a federated learning prediction framework, supporting logistic regression and XGBoost algorithms, covering single/batch processing, parameter validation, and result merging. It defines standardized processes through abstract classes and template methods, leveraging annotations and reflection for dynamic loading. Key components include model processors, algorithm managers, and thread pools, suitable for both real-time and offline prediction scenarios. |
