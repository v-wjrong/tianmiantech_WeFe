...
| Name | serving |
| Language | .java |
| Code Path | WeFe/serving/serving-sdk-java/src/main/java/com/welab/wefe/serving |
| Package Name | docs.serving.serving-sdk-java.src.main.java.com.welab.wefe.serving |
...
This flowchart illustrates the complete directory structure of the WeFe service SDK, starting from the root directory 'serving' and hierarchically expanding all components under the 'sdk' subdirectory. Core modules include processor, algorithm, predictor, utility classes (utils), enums, manager, data transfer objects (dto), model, and configuration (config). The algorithm module is further subdivided into logistic regression (lr) and XGBoost implementations, each containing both single-instance (single) and batch processing modes. The entire structure presents clear hierarchy, demonstrating typical modular division of a machine learning service SDK.

# File List

| Name   | Type  | Description |
|-------|------|-------------|
| [sdk](sdk/_module.md) | package | This module provides a federated learning prediction framework, supporting logistic regression and XGBoost algorithms, covering single/batch processing, parameter validation, and result merging. It defines standardized processes through abstract classes and template methods, leveraging annotations and reflection for dynamic loading. Key components include model processors, algorithm managers, and thread pools, suitable for both real-time and offline prediction scenarios. |
